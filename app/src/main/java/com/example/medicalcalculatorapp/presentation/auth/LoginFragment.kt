    ...
    private fun startGuestSession(isProfessional: Boolean, professionalType: String, licenseInfo: String?) {
        lifecycleScope.launch {
            try {
                ...
                // CRITICAL FIX: Initialize guest session in UserManager FIRST
                ...
                if (userManager.getUserType() != UserManager.UserType.GUEST) {
                    println("❌ Guest session initialization failed")
                    showError("Error inicializando sesión de invitado")
                    return@launch
                }

                // Record compliance for guest user
                val success = complianceManagerService.recordCompleteCompliance(
                    professionalType = if (isProfessional) {
                        mapToProfessionalType(professionalType)
                    } else {
                        com.example.medicalcalculatorapp.domain.service.SimpleProfessionalType.OTHER
                    },
                ...
                if (success) {
                    ...
                    println("❌ Compliance recording failed")
                    showError("Error configurando conformidad")
                }
...
// ====== ADDITIONAL DEBUG HELPER ======
    ...
    private fun debugGuestSessionState() {
        println("🔍 DEBUG Guest Session State:")
        println("   - UserType: ${userManager.getUserType()}")
        println("   - IsGuestMode: ${userManager.isGuestMode()}")
        println("   - GuestSessionInfo: ${userManager.getGuestSessionInfo()}")
    }
